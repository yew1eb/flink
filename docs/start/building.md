...
**By default, Flink is built with the Scala 2.11**. To build Flink with Scala *2.12*, you can change the default Scala *binary version* by using *scala-2.12* build profile:

~~~bash
# Build with Scala version 2.12
...
Flink is developed against Scala *2.11* and tested additionally against Scala *2.12*. These two versions are known to be compatible. Earlier versions (such as Scala *2.10*) are *not* compatible.
